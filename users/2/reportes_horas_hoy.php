...
//Data loading
@include '../../conex.php';
$sql = "SELECT v.ci, v.nombre, v.apellido, v.horas_diurnas, v.horas_nocturnas, v.horas_asistidas
        FROM v_asistencia as v, asistencias as a
        WHERE a.fecha = '$hoy'";
...
$tbl .= '<thead>
            <tr style="background-color:#efefef;font-weight: bold;" >
                ...
                <th style="border: 0px solid #000000; width: 80px; text-align: center;">HD</th>
                <th style="border: 0px solid #000000; width: 40px; text-align: center;">HN</th>
                <th style="border: 0px solid #000000; width: 174px; text-align: center;">TOTAL TRABAJADAS</th>
            </tr>
        </thead>';
if ($result = @mysql_query($sql)) {
    ...
    if ($num_rw > 0) {
        while ($A = @mysql_fetch_array($result)) {
            ...
            $nombre = $A["nombre"];
            $apellido = $A["apellido"];
            $fecha = $A["horas_diurnas"];
            $sexo = $A["horas_nocturnas"];
            $dpto = $A["horas_asistidas"];
// -----------------------------------------------------------------------------
            $tbl .= '
                    <tr>
